...
%global npm_bare      lefthook-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-lefthook-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Lefthook git hook configuration rules replacing heavy legacy hook managers

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/lefthook-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/lefthook-config is a shareable configuration preset with no runtime
...
install -pm 0644 lefthook.yml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
